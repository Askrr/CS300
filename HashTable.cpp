    ...
    if (oldNode == nullptr) {// if no entry found for the key
        ...
        if (oldNode->key == UINT_MAX) {// assing old node key to UNIT_MAX, set to key, set old node to bid and old node next to null pointer
            ...
            while (oldNode->next != nullptr) {
                oldNode = oldNode->next;// add new newNode to end
            }
...
void HashTable::PrintAll() {
    for (auto it = nodes.begin(); it != nodes.end(); ++it) {// for node begin to end iterate
        if (it->key != UINT_MAX) {//   if key not equal to UINT_MAx
            std::cout << "Key: " << it->key << ": " << it->bid.bidId << " + " << it->bid.title << " | " << it->bid.fund << endl;// output key, bidID, title, amount and fund
...
void HashTable::Remove(string bidId) {
    unsigned int key = hash(atoi(bidId.c_str()));// set key equal to hash atoi bidID cstring
    Node* node = &(nodes.at(key));
    if (node->key != UINT_MAX) {
        if (node->bid.bidId.compare(bidId) == 0) {
            //remove the node
            std::cout << "Remove top nodes for: " << bidId << std::endl;//print out what we're removing
            if (node->next == nullptr) {//if the next node is equal to nullptr
                node->key = UINT_MAX; //the key node will equal UINT_MAX
                return;
            }
            else {
                nodes.at(key) = *(node->next); //otherwise the node at the key will equal to the next node pointer
            }
        }
        else {
            Node* cur = node->next;
            ...
            while (cur != nullptr) {
                if (cur->bid.bidId.compare(bidId) == 0) {
                    //Delete the node
                    pre->next = cur->next; //previous node equals to current
                    delete cur; // delte the current
                    cur = nullptr; //the current is now nullptr
                    return;
                }
                pre = cur; // assign the pre pointer to the current
                cur = cur->next; // and current now points to next pointer
            }
        }
    }
}
...
Bid HashTable::Search(string bidId) {
    Bid bid;
    unsigned int key = hash(atoi(bidId.c_str()));// create the key for the given bid
    Node* node = &(nodes.at(key));
    ...
    while (node != nullptr) {// while node not equal to nullptr
        if (node->key != UINT_MAX && node->bid.bidId.compare(bidId) == 0) {// if the current node matches, return it
            return node->bid;//reutrn the bid
        }
